...
/*   Updated: 2025/12/26 05:41:14 by pmateo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Response*	doCgi( const Request& request, const Server& server, const std::string& path )
{
	int	pipe_parent[2];
	int	pipe_children[2];
	...
	else
	{
		...
		return (handleOutput(output_str));
	}
}

void	childExec( const Request& request, const Server& server, \
...
char**	createEnvp( const Request& request, const Server& server, const std::string& path )
{
	...
	envp_vec.push_back("SERVER_PORT=" + server.getPort());
	std::string clean_uri = request.getUri();
	size_t pos = clean_uri.find('?');
	if (pos != std::string::npos)
		clean_uri = clean_uri.substr(0, pos);
...
std::multimap<std::string, std::string>	handleOutputHeaders( const std::string& headers_str )
{
	...
	for (size_t i = 0; i < lines.size(); ++i)
	{
		...
		header_pair.first = Message::__toLower(line.substr(0, colon)); 
